/**
 * daily-activities.js
 * Logic untuk halaman Monitoring Data Aktivitas Harian (Admin Dashboard).
 * File ini MURNI JavaScript -- tidak ada syntax Blade di sini.
 * Data dinamis dari Laravel diambil dari window.DailyActivitiesConfig.
 *
...
(function () {
    ...
    var CFG = window.DailyActivitiesConfig || {};
    var CLOUD_NAME = 'kjcs8wz3';
    var UPLOAD_PRESET = '4putra_unsigned';
    var UPLOAD_FOLDER = '4putra/daily_activities';
    ...
    var els = {
        formCreate: document.getElementById('form-create-daily-activities'),
    ...
    function openDeleteModal(id, title) {
        ...
        els.formDelete.action = CFG.activitiesBaseUrl + '/' + id;
        showModal('delete');
    }

    function openEditModal(activity) {
        document.getElementById('edit-title').value = activity.title || '';
        document.getElementById('edit-title-en').value = activity.title_en || '';
        ...
        populateVideoList('edit-video-url-list', activity.video_urls || []);
    ...
    function renderExistingPhotos(images) {
        ...
        if (!images.length) {
            wrap.innerHTML = '<p class="text-xs text-gray-500 py-2">Belum ada foto tersimpan untuk aktivitas ini.</p>';
            return;
        }
    ...
    function submitWithUploads(formEl, files, doneBtnText, successTitle, successMsg) {
        var btn = formEl.id === 'form-create-daily-activities' ? submitCreateBtn : submitEditBtn;
        ...
        var uploadPromise = files.length > 0
            ? Promise.all(files.map(function (f) { return uploadToCloudinary(f, formEl.id === 'form-create-daily-activities' ? dzCreate : dzEdit); }))
            : Promise.resolve([]);
        ...
        return uploadPromise
            .then(function (urls) {
                ...
                return fetch(formEl.action, {
                    method: formEl.id === 'form-create-daily-activities' ? 'POST' : 'POST',
                    body: fd,
    ...
    if (submitCreateBtn) {
        submitCreateBtn.addEventListener('click', function () {
            ...
            submitWithUploads(
                ...
                'Simpan Data',
                'Berhasil!',
                'Aktivitas harian baru telah ditambahkan.'
            );
        });
    }
    ...
    if (confirmUpdateBtn && submitEditBtn) {
        confirmUpdateBtn.addEventListener('click', function () {
            ...
            submitWithUploads(
                ...
                'Perbarui Data',
                'Berhasil Diubah!',
                'Aktivitas harian telah diperbarui.'
            );
        });
    }
