...
@section('content')
    ...
    <div class="mb-4">
        <div class="relative max-w-md">
            ...
            <input type="text" class="table-search-input w-full bg-[#151a22] border border-gray-700 rounded-xl pl-10 pr-4 py-2.5 text-sm text-white placeholder-gray-500 focus:border-[#E62C37] focus:outline-none" placeholder="Cari portofolio...">
        </div>
    </div>
                        ...
                        <tr class="hover:bg-gray-800/50 transition-colors duration-150">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-4">
                                    ...
                                    @else
                                        <div
                                            class="w-11 h-11 rounded bg-gray-800 border border-gray-700 flex items-center justify-center">
    ...
    <x-admin.modal id="create" maxWidth="max-w-2xl">
        ...
        <form id="form-create-achievements" action="{{ route('admin.achievements.store') }}" method="POST"
            ...
            @csrf
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-gray-400 mb-1">Judul
                    Portofolio (ID)</label>
            ...
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-gray-400 mb-1">Judul Portofolio
                    Terjemahan (EN)</label>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-gray-400 mb-1">Tahun
                        Event</label>
                ...
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-gray-400 mb-1">Tanggal
                        Pelaksanaan</label>
                    <input type="date" id="create-input-date" name="date"
                        class="w-full p-2.5 text-sm bg-[#151a22] border border-gray-700 text-white focus:outline-none focus:border-[#E62C37] cursor-pointer [color-scheme:dark]">
                    <span id="error-create-date" class="hidden text-xs font-semibold text-[#E62C37] mt-1">Tanggal
                        pelaksanaan wajib dipilih</span>
                </div>
            </div>
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-gray-400 mb-1">Deskripsi
                    Lengkap</label>
                <textarea id="create-input-description" name="description" rows="4"
                    class="w-full p-2.5 text-sm bg-[#151a22] border border-gray-700 rounded-xl text-white focus:outline-none focus:border-[#E62C37]"></textarea>
            ...
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-gray-400 mb-1">Deskripsi Lengkap
                    Terjemahan (EN)</label>
            ...
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-gray-400 mb-1">Link Video
            ...
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-gray-400 mb-1">Link Eksternal
            ...
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-gray-400 mb-1">Upload Dokumentasi
                ...
                <div id="dropzone-create"
                    ...
                    <div class="dz-message text-sm text-gray-400">
        ...
        <form id="form-edit-action" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="date" name="date" id="edit-date"
                        class="w-full p-2.5 text-sm bg-[#151a22] border border-gray-700 text-white focus:outline-none focus:border-amber-500 cursor-pointer [color-scheme:dark]">
            ...
            <div>
                ...
                <textarea name="description" id="edit-description" rows="4"
                    class="w-full p-2.5 text-sm bg-[#151a22] border border-gray-700 rounded-xl text-white focus:outline-none focus:border-amber-500"></textarea>
            ...
            <div id="edit-existing-video-wrap" class="hidden">
                <label class="block text-xs font-bold uppercase tracking-wider text-gray-400 mb-1">Video Tersimpan</label>
                <div id="edit-existing-video"
                    class="flex items-center gap-3 p-3 bg-[#151a22] border border-gray-700 rounded">
                    <svg class="w-5 h-5 text-amber-500 shrink-0" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M8 5v14l11-7z" />
                    </svg>
                    <span id="edit-video-name" class="text-xs text-gray-300 truncate flex-1"></span>
            ...
            <div>
                ...
                <div id="edit-existing-photos"
                    class="flex flex-wrap gap-2 p-3 bg-[#151a22] border border-gray-700 rounded min-h-[70px]">
            ...
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-gray-400 mb-1">Tambah Foto/Video
    ...
    <x-admin.modal id="confirm-update" maxWidth="max-w-md" zIndex="z-[60]">
        <div class="flex items-center gap-3 mb-4">
            ...
            <div>
                <h3 class="text-lg font-bold text-white">Konfirmasi Pembaruan</h3>
                <p class="text-sm text-gray-400">Pastikan data sudah benar</p>
            </div>
        </div>
        <p class="text-sm text-gray-300 mb-6">Lanjutkan pembaruan data portofolio?</p>
        <div class="flex gap-3 justify-end">
            <button type="button" onclick="closeModal('confirm-update')"
                class="px-4 py-2.5 text-sm font-semibold text-gray-300 bg-[#151a22] border border-gray-600 rounded-xl hover:border-gray-500 transition-colors">Periksa
                Lagi</button>
    ...
    <x-admin.modal-confirm-delete id="delete" formId="form-delete-action" title="Konfirmasi Penghapusan">
        Apakah Anda yakin menghapus portofolio <span id="delete-target-name" class="font-bold text-white"></span>
    ...
    <x-admin.modal id="confirm-delete-photo" maxWidth="max-w-md" zIndex="z-[60]">
        ...
        <p class="text-sm text-gray-300 mb-6">Apakah Anda yakin menghapus foto ini secara permanen?</p>
        <div class="flex gap-3 justify-end">
    ...
    <x-admin.modal id="confirm-delete-video" maxWidth="max-w-md" zIndex="z-[60]">
        ...
        <div class="flex items-center gap-3 mb-4">
            ...
            <div>
                <h3 class="text-lg font-bold text-white">Hapus Video</h3>
                <p class="text-sm text-gray-400">Tindakan ini tidak dapat dibatalkan</p>
            </div>
        </div>
        <p class="text-sm text-gray-300 mb-6">Apakah Anda yakin menghapus video yang tersimpan?</p>
        <div class="flex gap-3 justify-end">
