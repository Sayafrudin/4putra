...
@section('content')
    <div class="my-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-gray-800 pb-5">
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-white uppercase">Transaksi Chatbot</h2>
            <p class="text-sm text-gray-400 mt-1">Daftar transaksi pembayaran pelanggan WhatsApp</p>
        </div>
        <div class="flex items-center gap-3 flex-wrap">
            ...
            <a href="{{ route('admin.chatbot.index') }}"
                class="inline-block px-4 py-2.5 text-sm font-bold text-gray-300 border border-gray-600 hover:border-gray-500 rounded-xl transition-colors">
                Kembali
            </a>
        </div>
    </div>
    ...
    <div class="mb-4">
        <div class="relative max-w-md">
            ...
            <input type="text" class="table-search-input w-full bg-[#151a22] border border-gray-700 rounded-xl pl-10 pr-4 py-2.5 text-sm text-white placeholder-gray-500 focus:border-[#E62C37] focus:outline-none" placeholder="Cari transaksi...">
        </div>
    </div>

    {{-- Statistik --}}
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div class="bg-[#1e2530] border border-gray-800 rounded-xl px-5 py-4">
        ...
        <div class="bg-[#1e2530] border border-gray-800 rounded-xl px-5 py-4">
            ...
            <p class="text-2xl font-bold text-white mt-1">Rp {{ number_format($totalRevenue ?? 0, 0, ',', '.') }}</p>
        </div>
    </div>
    ...
    <div class="w-full overflow-hidden bg-[#1e2530] border border-gray-800 shadow-sm mb-10 rounded-xl table-search-wrapper">
            ...
            <table class="table-searchable w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-800">
                    @forelse($transaksi as $trx)
                        <tr class="text-gray-300 hover:bg-[#262d3a] transition-colors" data-order-id="{{ $trx->midtrans_order_id }}" data-trx-id="{{ $trx->id }}">
                            <td class="px-6 py-4 text-xs font-mono text-gray-400">{{ $trx->midtrans_order_id ?? '-' }}</td>
                            <td class="px-6 py-4 text-sm">
                                <div class="font-medium text-white">{{ $trx->pelanggan->nama ?? '-' }}</div>
                                @php
                                    $nomorRaw = str_replace(['@s.whatsapp.net', '@lid'], '', $trx->pelanggan->nomor_wa ?? '');
                                    $isLid = str_contains($trx->pelanggan->nomor_wa ?? '', '@lid');
                                @endphp
                                <div class="text-xs text-gray-400">{{ $isLid ? 'WA ID: ' . $nomorRaw : $nomorRaw }}</div>
                            </td>
                            <td class="px-6 py-4 text-sm">
                                @if($trx->inventaris)
                                    <div class="font-medium text-white">{{ $trx->inventaris->nama_spesies }}</div>
                                    <div class="flex items-center gap-2 mt-1">
                                        ...
                                        <span class="text-xs text-gray-400">Rp {{ number_format($trx->inventaris->harga, 0, ',', '.') }}/ekor</span>
                                    </div>
                                @else
                            ...
                            <td class="px-6 py-4 text-sm font-medium text-center">
                                <span class="px-2 py-1 bg-gray-600/20 rounded-lg text-white">{{ $trx->quantity ?? 1 }}</span>
                            </td>
                            <td class="px-6 py-4 text-sm font-bold text-white">Rp {{ number_format($trx->total_harga, 0, ',', '.') }}</td>
                            <td class="px-6 py-4 text-sm" id="status-{{ $trx->id }}">
                                ...
                                @elseif($trx->status === 'expired')
                                    <span class="px-2 py-1 rounded-full text-xs font-semibold bg-gray-500/20 text-gray-400">Kadaluarsa</span>
                                @else
                                    <span class="px-2 py-1 rounded-full text-xs font-semibold bg-red-500/20 text-red-400">Dibatalkan</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-xs text-gray-400">{{ $trx->created_at->format('d M Y H:i') }}</td>
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    {{-- Dropdown ubah status --}}
                                    <select onchange="ubahStatus({{ $trx->id }}, this.value)" class="text-xs bg-[#151a22] border border-gray-700 rounded-lg px-2 py-1.5 text-gray-300 focus:outline-none focus:border-[#E62C37] cursor-pointer">
    ...
    <div id="modalExportPdf" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/60 backdrop-blur-sm">
        <div class="bg-[#1e2530] border border-gray-700 rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6 text-center">
            ...
            <h3 class="text-lg font-bold text-white mb-2">Export PDF</h3>
            <p class="text-sm text-gray-400 mb-6">Download laporan transaksi dalam format PDF dengan desain invoice.</p>
    ...
    <div id="modalExportExcel" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/60 backdrop-blur-sm">
        <div class="bg-[#1e2530] border border-gray-700 rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6 text-center">
            ...
            <h3 class="text-lg font-bold text-white mb-2">Export Excel</h3>
            <p class="text-sm text-gray-400 mb-6">Download data transaksi dalam format CSV (Excel).</p>
            <div class="flex gap-3 justify-center">
