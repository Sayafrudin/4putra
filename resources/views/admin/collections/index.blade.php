...
@section('content')
    <div class="my-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-gray-200 dark:border-gray-800 pb-5">
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white uppercase">Manajemen Koleksi Burung</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Kelola data koleksi burung yang tampil di halaman publik</p>
        </div>
    ...
    <div class="mb-4">
        <div class="relative max-w-md">
            ...
            <input type="text" class="table-search-input w-full bg-white dark:bg-[#151a22] border border-gray-300 dark:border-gray-700 rounded-xl pl-10 pr-4 py-2.5 text-sm text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:border-[#E62C37] focus:outline-none" placeholder="Cari koleksi...">
        </div>
    </div>
                ...
                <tbody data-admin-list class="divide-y divide-gray-200 dark:divide-gray-800 text-gray-600 dark:text-gray-300">
                    @forelse($collections as $collection)
                        <tr class="hover:bg-gray-800/50 transition-colors duration-150">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-4">
                                    @if ($collection->image_path)
                                        @php
                                            $colImg = str_starts_with($collection->image_path, 'http')
                                                ? str_replace('/upload/', '/upload/w_100,c_fill,q_auto,f_auto/', $collection->image_path)
                                                : asset('storage/collections/' . $collection->image_path);
                                        @endphp
                                        <img src="{{ $colImg }}"
                                            class="w-12 h-12 rounded border border-gray-300 dark:border-gray-700 object-cover cursor-pointer hover:scale-125 hover:z-10 hover:border-[#E62C37] shadow-sm transition-all duration-200 relative"
                                            onclick="zoomImage(this.src)" alt="{{ $collection->name }}">
                                    @else
                                        <div class="w-12 h-12 rounded bg-gray-800 border border-gray-300 dark:border-gray-700 flex items-center justify-center">
                            ...
                            <td class="px-6 py-4">
                                <span class="px-2.5 py-1 text-xs font-bold border border-gray-300 dark:border-gray-700 bg-white dark:bg-[#151a22] rounded">{{ $collection->category }}</span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <span class="text-xs font-mono text-gray-500 dark:text-gray-400">{{ $collection->sort_order }}</span>
                            </td>
    ...
    <x-admin.modal id="create-collection" maxWidth="max-w-2xl">
        ...
        <form id="form-create-collection" action="{{ route('admin.collections.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1">Nama Burung (EN)</label>
                    <input type="text" name="name_en"
                        class="w-full p-2.5 text-sm bg-white dark:bg-[#151a22] border border-gray-300 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:outline-none focus:border-[#E62C37]">
                </div>
            </div>
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1">Nama Ilmiah</label>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1">Kategori (EN)</label>
    ...
    <x-admin.modal id="edit-collection" maxWidth="max-w-2xl">
        ...
        <form id="form-edit-collection" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1">Urutan (Sort Order)</label>
                <input type="number" id="edit-col-sort-order" name="sort_order" min="0" value="0" placeholder="0"
                    class="w-full p-2.5 text-sm bg-white dark:bg-[#151a22] border border-gray-300 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:outline-none focus:border-amber-500">
                <p class="text-xs text-gray-500 mt-1">Angka lebih kecil = tampil lebih dulu. Default: 0</p>
            </div>
            {{-- Foto tersimpan --}}
    ...
    <x-admin.modal-confirm-delete id="delete-collection" formId="form-delete-collection" title="Konfirmasi Hapus Koleksi">
        Apakah Anda yakin menghapus koleksi <span id="delete-collection-name" class="font-bold text-gray-900 dark:text-white"></span>
    ...
    <x-admin.modal id="confirm-delete-col-photo" maxWidth="max-w-md" zIndex="z-[60]">
        ...
        <p class="text-sm text-gray-600 dark:text-gray-300 mb-6">Apakah Anda yakin menghapus foto koleksi ini secara permanen?</p>
