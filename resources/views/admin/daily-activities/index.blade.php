...
@section('content')
    <div
        ...
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white uppercase">
                Monitoring Data Aktivitas Harian
            </h2>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                Manajemen jurnal kegiatan harian penangkaran PT 4Putra Vertex Aviary secara real-time
            </p>
        </div>
    ...
    <div class="mb-4">
        <div class="relative max-w-md">
            ...
            <input type="text" class="table-search-input w-full bg-white dark:bg-[#151a22] border border-gray-300 dark:border-gray-700 rounded-xl pl-10 pr-4 py-2.5 text-sm text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:border-[#E62C37] focus:outline-none" placeholder="Cari aktivitas...">
        </div>
    </div>
            ...
            <table class="table-searchable w-full min-w-[640px] text-left border-collapse">
                <thead>
                    <tr
                        ...
                        <th class="px-4 sm:px-6 py-4 w-[55%]">Informasi Aktivitas / Judul</th>
                        <th class="px-4 sm:px-6 py-4 w-32 sm:w-44">Tanggal Aktivitas</th>
                        ...
                        <tr class="hover:bg-gray-800/50 transition-colors duration-150">
                                ...
                                <div class="flex items-center gap-3 sm:gap-4">
                                    @if (!empty($activity->images))
                                        <div class="flex flex-wrap gap-1.5 max-w-[180px] shrink-0">
                                            @foreach (array_slice($activity->images, 0, 3) as $img)
                                                @php
                                                    $actImg = str_starts_with($img, 'http')
                                                        ? str_replace('/upload/', '/upload/w_100,h_100,c_fill,q_auto,f_auto/', $img)
                                                        : asset('storage/daily-activities/' . $img);
                                                @endphp
                                                <img src="{{ $actImg }}"
                                                    ...
                                                    alt="Foto {{ $activity->title }}" title="Klik untuk preview"
                                                    loading="lazy">
                                            @endforeach
                                            @if (count($activity->images) > 3)
                                                <span
                                                    ...
                                                    title="Total {{ count($activity->images) }} foto">+{{ count($activity->images) - 3 }}</span>
                                            @endif
                                        </div>
                                    @else
                                        <div
                            ...
                            <td class="px-4 sm:px-6 py-4 text-right">
                                <div class="inline-flex items-center gap-2 whitespace-nowrap">
                                    <button onclick="openEditModal({{ json_encode($activity) }})"
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="px-4 sm:px-6 py-12 text-sm text-center text-gray-500 bg-white dark:bg-[#151a22]">
                                Sistem mendeteksi belum ada aktivitas harian yang tersimpan di dalam database
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <x-admin.modal id="create" maxWidth="max-w-2xl">
        ...
        <div class="flex items-center justify-between mb-6 pb-2 border-b border-gray-200 dark:border-gray-800 pr-12">
            <h3 class="text-lg font-bold uppercase tracking-wide text-gray-900 dark:text-white flex items-center gap-2">
                <span class="w-2 h-2 bg-[#E62C37]"></span> Formulir Input Aktivitas Harian Baru
            </h3>
        </div>

        <form id="form-create-daily-activities" action="{{ route('admin.daily-activities.store') }}" method="POST"
            enctype="multipart/form-data" class="space-y-4">
            @csrf
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1">Judul
                    Aktivitas (ID)</label>
                <input type="text" id="create-input-title" name="title" placeholder="Contoh: Rotasi Pakan Pagi dan Penataan Enrichment"
            ...
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1">Judul Aktivitas
                    Terjemahan (EN)</label>
                <input type="text" name="title_en" placeholder="Example: Morning Feed Rotation and Enrichment Setup"
                    class="w-full p-2.5 text-sm bg-white dark:bg-[#151a22] border border-gray-300 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:outline-none focus:border-[#E62C37]">
            </div>
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1">Tanggal
                    Aktivitas</label>
                <input type="date" id="create-input-date" name="activity_date"
                    class="w-full p-2.5 text-sm bg-white dark:bg-[#151a22] border border-gray-300 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:outline-none focus:border-[#E62C37] cursor-pointer dark:[color-scheme:dark]">
                <span id="error-create-date" class="hidden text-xs font-semibold text-[#E62C37] mt-1">Tanggal aktivitas
                    wajib dipilih</span>
            </div>
            <div>
                ...
                <textarea id="create-input-description" name="description" rows="5"
                    placeholder="Ceritakan jalannya aktivitas: jam mulai, kandang yang ditangani, hasil pencatatan, dan tindak lanjutnya."
                ...
                <span id="error-create-description" class="hidden text-xs font-semibold text-[#E62C37] mt-1">Deskripsi
                    aktivitas tidak boleh kosong</span>
            </div>
            <div>
    ...
    <x-admin.modal id="edit" maxWidth="max-w-2xl">
        ...
        <form id="form-edit-action" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1">Tanggal
                    Aktivitas</label>
                <input type="date" name="activity_date" id="edit-date"
                    class="w-full p-2.5 text-sm bg-white dark:bg-[#151a22] border border-gray-300 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:outline-none focus:border-amber-500 cursor-pointer dark:[color-scheme:dark]">
                <span id="error-edit-date" class="hidden text-xs font-semibold text-[#E62C37] mt-1">Tanggal aktivitas
                    wajib dipilih</span>
            </div>
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1">Deskripsi
            ...
            <div>
                ...
                <div id="edit-existing-photos"
                    ...
                    {{-- diisi dinamis oleh daily-activities.js --}}
                </div>
    ...
    <x-admin.modal id="confirm-update" maxWidth="max-w-md" zIndex="z-[60]">
        ...
        <p class="text-sm text-gray-600 dark:text-gray-300 mb-6">Lanjutkan pembaruan data aktivitas harian?</p>
    ...
    <x-admin.modal-confirm-delete id="delete" formId="form-delete-action" title="Konfirmasi Penghapusan">
        Apakah Anda yakin menghapus aktivitas harian <span id="delete-target-name" class="font-bold text-gray-900 dark:text-white"></span>
    ...
    <script>
        window.DailyActivitiesConfig = {
            csrfToken: document.querySelector('meta[name="csrf-token"]')?.content ?? '{{ csrf_token() }}',
            storeUrl: @json(route('admin.daily-activities.store')),
            activitiesBaseUrl: @json(url('/admin/daily-activities')),
            storageUrl: @json(asset('storage/daily-activities')),
            flash: {
